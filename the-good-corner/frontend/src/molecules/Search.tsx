...
import { useGetAdsByNeedleLazyQuery } from "../libs/graphql/generated/graphql-types";

export default function Search() {
	const [needle, setNeedle] = useState<string>("");
	const [searchAds, { loading, error, data }] = useGetAdsByNeedleLazyQuery();

	const hChange = (evt: FormEvent<HTMLInputElement>) => {
		setNeedle(evt.currentTarget.value);
		if (evt.currentTarget.value) {
			searchAds({ variables: { needle: evt.currentTarget.value } });
		} else {
			//TODO
			// data = undefined;
		}
	...
	return (
		<>
			<form className="text-field-with-button">
				...
				{!data && (
					<ul>
						<li>"No content found, sorry é_è"</li>
					</ul>
				)}
				{!loading && !error && data && (
					<ul>
						{data.getAdsByNeedle.map((res) => {
							return (
								<li key={res.id}>
									<Link to={`/ads/${res.id}`}>{res.title}</Link>
								</li>
							);
						})}
					</ul>
